...
function Privados(){
    ...
    async function masCercano (){
        var mas_cercano = 0;
            ...
            var best_dif = 9999;
            var dif;
            for(let i = 1; i < privados.length; ++i){
                dif = Math.abs(point.lat - privados[i].lat) +
                        Math.abs(point.lng - privados[i].lng);
                if(best_dif > dif){
                    best_dif = dif;
                    best = privados[i];
                }
    ...
    return (
        <div>
            ...
            <div class="container">
                <h1 class="display-4">Atencion Medica - HOSPITALES PRIVADOS</h1>
            ...
            {
                state.longitude === "" ? null : <Maps longitude={state.longitude} latitude={state.latitude} places={privados} url={state.url} mas_cercano={state.mas_cercano}/> 
            }
