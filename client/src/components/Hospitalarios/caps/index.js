...
function Caps(){
    ...
    async function masCercano (){
        var mas_cercano = 0;
            ...
            for(let i = 1; i < caps.length; ++i){
                dif = Math.abs(point.lat - caps[i].lat) +
                        Math.abs(point.lng - caps[i].lng);
                if(best_dif > dif){
                    best_dif = dif;
                    best = caps[i];
                }
    ...
    return (
        <div>
            ...
            <div class="container">
                <h1 class="display-4">Atencion Medica - CAPS</h1>
            ...
            {
                state.longitude === "" ? null : <Maps longitude={state.longitude} latitude={state.latitude} places={caps} url={state.url} mas_cercano={state.mas_cercano}/> 
            }
